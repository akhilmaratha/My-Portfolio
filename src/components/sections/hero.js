...
function CodeSnippetCard() {
  return (
    <div className="relative overflow-hidden rounded-4xl border border-white/10 bg-[#091017] p-4">
      <div className="mb-4 flex items-center gap-2 border-b border-white/10 pb-3 text-xs text-white/50">
        <span className="h-3 w-3 rounded-full bg-[#ff5f57]" />
        <span className="h-3 w-3 rounded-full bg-[#febc2e]" />
        <span className="h-3 w-3 rounded-full bg-[#28c840]" />
        <span className="ml-2 font-mono">server/routes/contact.js</span>
      </div>
      ...
      <div className="mx-auto grid max-w-7xl gap-14 px-4 pb-20 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:px-8 lg:pb-28">
        <div className="relative z-10 max-w-3xl">
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.4 }} className="mt-10 flex flex-wrap gap-4">
            <MagneticButton as="a" href="#projects" className="bg-[#00ffaa] text-[#071014] hover:text-black">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.5 }} className="mt-12 grid gap-3 sm:grid-cols-3">
            {['Next.js','MongoDB','Framer Motion'].map((item) => (
              <div key={item} className="rounded-2xl border border-white/10 bg-white/3 px-4 py-3 font-mono text-xs uppercase tracking-[0.24em] text-white/70">
                {item}
              </div>
            ))}
          </motion.div>

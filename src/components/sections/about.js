    ...
    <section id="about" className="relative overflow-hidden px-4 py-24 sm:px-6 lg:px-8 lg:py-32">
      <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1.1fr_0.9fr]">
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          ...
          <h2 className="mt-4 max-w-xl font-syne text-4xl font-black tracking-tight text-white sm:text-5xl">I build interfaces that feel alive and systems that stay maintainable.</h2>
        ...
        <div className="space-y-6">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="grid gap-4 sm:grid-cols-2">
            {infoCards.map((card) => (
              <div key={card.label} className="group rounded-2xl border border-white/10 border-l-[#00ffaa] `bg-white/3` p-5 transition-transform duration-300 hover:translate-x-1">
